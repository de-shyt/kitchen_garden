...
void Network::receive(std::vector<std::unique_ptr<CoFarmer>>& enemies, Player* p) {
		...
		if (type == 0) { // you connected to server, get your ID
			...
			for (unsigned int i = 0; i < enemies.size(); i++) {
				if (enemies[i]->getID() == id) {
					...
					std::cout << "CoFarmer: " << enemies[i]->getID() << " deleted " << std::endl;
					enemies.erase(enemies.begin() + i);
				}
			}
		} else if (type == 2) {
			std::cout << "Server is full" << std::endl;
		} else if (type == 3) { // get move direction of cofarmers
			for (unsigned int i = 0; i < enemies.size(); i++) {
				if (enemies[i]->getID() == id) {
					sf::Vector2f dir;
					receivePacket >> dir.x;
		...
		//Number 6 is reserved by the server for saving the name
			...
			for (unsigned int i = 0; i < playersId.size(); ++i) { //loop through id-s we got
				...
				if (playersId[i] != p->getID() && !haveThatEnemy) { //if it is not our id and if we dont have that enemy, create a new enemy with that id
					enemies.push_back(std::make_unique<CoFarmer>(playersId[i], sf::Vector2f(100, 100), playersName[i]));
					m_textMessage = "New player connected: " + playersName[i];
					std::cout << "Created a new CoFarmer with id: " << playersId[i] << std::endl;
				}
			}
